...
import phenix.model.{CompleteDayKpi, FileOutput, KpiOutput}
...
//    lazy val transactionFileName = "data/input/simple-example/transactions_20170514.data"
    ...
    transactions.map(transactions => {
      ...
      FileOrchestrator.outputCompleteDayKpi(sortedDayKpiResults) // TODO Add truncated results
    })
  }
}
...
object FileOrchestrator extends LazyLogging with FileProducer {
  def outputCompleteDayKpi(completeDayKpi: CompleteDayKpi): Unit = {
    ...
    outputKpis(KpiOutput(Paths.get("data/output/"), completeFileOutputs))
  }
